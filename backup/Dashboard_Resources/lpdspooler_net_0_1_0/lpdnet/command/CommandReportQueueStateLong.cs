...
namespace sf.net.lpdnet.command
{
	...
	/// </p>
	/// </summary>
	public class CommandReportQueueStateLong : CommandHandler
	{
		private static ILog log = LogManager.GetLogger(typeof (CommandReportQueueStateLong));
		...
		public CommandReportQueueStateLong(byte[] command, StreamReader reader, StreamWriter writer)
		...
		public override void execute()
		{
			...
			if (null != info && info.Count > 0)
			{
				...
				try
				{
					...
					sb.Append(StringUtil.createFixedLengthString("Date", PrintJob.JOB_DATE_LENGTH));
					sb.Append(StringUtil.createFixedLengthString("Size", PrintJob.JOB_SIZE_LENGTH));
					sb.Append("\n");
					...
					if (0x4 != cmd[0])
					{
						throw new LPDException("Cmd[0]=" + cmd[0] + ",should of been 0x4");
					}
					else
					{
						log.Debug("Report Queue State Command Long");
						// write ASCII string to output stream
						log.Debug("Print Queue status long=" + sb.ToString());
